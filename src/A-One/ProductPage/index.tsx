...
import { PRODUCT_PAGE_TABS, PRODUCT_PAGE_SCIENCE_TEXT, PRODUCT_PAGE_VALUE_PROPS } from './data';
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {PRODUCTS.filter(p => p.id !== id).slice(0, 3).map((p) => (
              <motion.div 
                ...
                className="space-y-6"
              >
                <Link to={`/product/${p.id}`} className="block aspect-[4/5] rounded-[3rem] overflow-hidden border border-slate-100 shadow-xl group bg-white">
                  <img
                    src={p.image}
                    alt={p.name}
                ...
                <div className="flex justify-between items-start px-4">
                  <div>
                    ...
                    <h3 className="text-xl font-light text-slate-900 mt-1 italic font-serif">
                      <Link to={`/product/${p.id}`} className="hover:opacity-70 transition-opacity">{p.name}</Link>
                    </h3>
                  </div>
