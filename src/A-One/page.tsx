...
import { ArrowRight, ChevronRight, ShoppingBag, Sparkles, Beaker } from 'lucide-react';
  ...
  return (
    <div className="bg-gradient-to-br from-white via-white to-accent-brand/20 min-h-screen text-slate-900 overflow-x-hidden">
      ...
      <section className="relative h-screen flex items-center z-10">
        <div className="max-w-7xl mx-auto px-6 w-full grid grid-cols-1 lg:grid-cols-2 items-center gap-16">
            ...
            >
              <div className="space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] uppercase tracking-[0.5em] text-slate-400 font-bold">Featured Collection</span>
                </div>
                <h1 className="text-6xl md:text-8xl font-light tracking-tight text-slate-900 leading-[0.95]">
                  {HERO_SLIDES[index].title} <br />
                  <span className="italic font-serif text-luxury-slate">{HERO_SLIDES[index].subtitle}</span>
                </h1>
              ...
              <div className="flex flex-wrap items-center gap-8">
                <Link
                  to="/shop"
                  className="group px-12 py-5 bg-luxury-dark text-white font-bold rounded-full text-[11px] uppercase tracking-[0.2em] hover:bg-luxury-slate transition-all duration-500 flex items-center gap-3"
                >
        ...
        <button
          onClick={next}
          className="absolute right-12 bottom-12 p-6 rounded-full border border-slate-200 bg-white/80 backdrop-blur-xl hover:bg-slate-900 hover:text-white transition-all group z-20 shadow-lg"
        >
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-6 space-y-48 py-32">
        <div className="text-center space-y-4">
          <div className="flex justify-center items-center gap-4 mb-4">
            ...
            <span className="text-[10px] uppercase tracking-[0.5em] text-accent-brand font-bold">Signature Series</span>
            <div className="h-px w-8 bg-slate-200" />
          </div>
          <h2 className="text-6xl md:text-8xl font-light text-slate-900 tracking-tighter">
            Curated <span className="italic font-serif text-luxury-slate">Excellence</span>
          </h2>
        </div>

        {PRODUCTS.slice(0, 3).map((product, idx) => (
          <div key={product.id} className={cn("flex flex-col md:flex-row items-center gap-24", idx % 2 !== 0 && "md:flex-row-reverse")}>
            <motion.div
            ...
            >
              <div className="space-y-6">
                <span className="text-[10px] uppercase tracking-widest text-accent-brand font-black">{product.category}</span>
                <h3 className="text-5xl md:text-7xl font-light text-slate-900 leading-[1.1] tracking-tight">{product.name}</h3>
                <p className="text-4xl font-serif text-luxury-slate">${product.price.toFixed(2)}</p>
              </div>
              <p className="text-slate-500 text-xl font-light leading-relaxed max-w-lg">{product.description}</p>
              <Link
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
          ...
          >
            <div className="aspect-[4/5] rounded-[4rem] overflow-hidden border border-slate-100 shadow-2xl">
              <img src="/assets/routine.png" alt="Routine" className="w-full h-full object-cover" />
            </div>
          </motion.div>
          ...
          <div className="space-y-12">
            <div className="space-y-8">
              <div className="flex items-center gap-4">
                <div className="h-px w-8 bg-slate-200" />
                <span className="text-[10px] uppercase tracking-[0.5em] text-accent-brand font-bold">Scientific Excellence</span>
              </div>
              <h2 className="text-7xl md:text-8xl font-light text-slate-900 leading-[1] tracking-tighter">
                Beauty Tailored <br />
                <span className="italic font-serif text-luxury-slate">To Your DNA.</span>
              </h2>
            </div>
            <Link
              to="/contact"
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-10 w-full grid grid-cols-1 lg:grid-cols-2 gap-32 items-center">
          ...
          >
            <div className="inline-flex items-center gap-4 px-6 py-2 rounded-full border border-slate-100 bg-white shadow-sm">
              ...
              <span className="text-[10px] uppercase tracking-[0.4em] text-slate-600 font-bold">The Heritage</span>
            </div>
            <h2 className="text-7xl md:text-9xl font-light leading-[0.9] tracking-tighter text-slate-900">
              Purity in <br />
              <span className="italic font-serif text-luxury-slate">Every Essence.</span>
            </h2>
            <div className="grid grid-cols-2 gap-16 pt-12 border-t border-slate-100">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}>
                <span className="text-6xl font-light text-slate-900 tracking-tighter">100%</span>
                <p className="text-[10px] uppercase tracking-[0.3em] text-accent-brand font-black">Traceable Origin</p>
              </motion.div>
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.4 }}>
                <span className="text-6xl font-light text-slate-900 tracking-tighter">Zero</span>
                <p className="text-[10px] uppercase tracking-[0.3em] text-accent-brand font-black">Synthetics Used</p>
              </motion.div>
            </div>
          </motion.div>
